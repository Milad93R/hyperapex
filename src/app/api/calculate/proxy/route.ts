...
/**
 * Server-side proxy for calculate API
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Forward the request to the calculate endpoint with server-side API key
    const response = await fetch(`${request.nextUrl.origin}/api/calculate`, {
      method: 'POST',
      headers: {
